...
public class MainActivity extends BaseActivity<ActivityMainBinding> {
    ...
    protected void initView() {
        if (Build.VERSION.SDK_INT >= 23) {
            if (SplashActivity.checkAndRequestPermission(this)) {
                ...
                mViewBinding.btnPermission.setText("PERMISSIONS：ON");
            }
        } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == SplashActivity.REQUEST_PERMISSION_CODE && SplashActivity.hasAllPermissionsGranted(grantResults)) {
            havePermissions = true;
            mViewBinding.btnPermission.setText("PERMISSIONS：ON");
        } else {
